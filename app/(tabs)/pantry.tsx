...
const MEMBER_COLORS = ["#dcecff", "#e6f3ff", "#eef8f1", "#f3f7ff", "#eaf4ff"];
...
export default function Pantry() {
  ...
  const totalItems = foodItems.reduce((sum, item) => sum + getItemCountValue(item), 0);
  ...
  const getOwnerLabel = (item: FoodItem) => {
    const ownerIds = getAssignedUserIds(item);
    const ownerNames = ownerIds
      .map((userId) => members.find((member) => member.user_id === userId)?.profiles?.display_name)
    ...
    return `Shared: ${ownerNames.join(", ")}`;
  };

  const getOwnerColorIndex = (item: FoodItem) => {
    const firstOwnerId = getAssignedUserIds(item)[0];
    const idx = members.findIndex((m) => m.user_id === firstOwnerId);
    return idx >= 0 ? idx : 0;
  };
  ...
  const handleDeleteItem = () => {
    ...
    Alert.alert(
      "Delete item?",
      `Remove ${itemToDelete.product_name} from the pantry?`,
      [
          ...
          <View style={styles.itemList}>
            {sortedItems.map((item) => (
              <FoodItemCard
                key={item.id}
                item={item}
                ownerName={getOwnerLabel(item)}
                ownerColorIndex={getOwnerColorIndex(item)}
      ...
      >
        ...
        >
          <Pressable style={styles.modalCard} onPress={() => {}}>
            ...
            <Text style={styles.modalBullet}>
              • Tap any item card to change quantity, dates, or photo.
            </Text>
            <Text style={styles.modalBullet}>
